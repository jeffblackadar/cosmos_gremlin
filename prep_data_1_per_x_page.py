...
df = pd.read_csv("wwper_x_page.csv", sep=";", header=0)

# open a file to write out the Gremlin statements
f = open("data_load_data_per_x_page.py", "w")
...
for index, row in df.iterrows():
    ...
    x_row = "g.V('per"+clean_string(str(row['id_entities_person']))+"').addE('appears').to(g.V('page"+clean_string(str(row['id_source_document']))+"'))"
